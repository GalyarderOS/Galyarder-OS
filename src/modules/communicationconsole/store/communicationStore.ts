...
import { CommunicationState, ContentPiece, VoiceDraft, BrandMetrics, PublicationGoal, VoiceVersion } from '../types'

const mockContentQueue: ContentPiece[] = [
  {
    id: '1',
...
const mockVoiceDrafts: VoiceDraft[] = [
  {
    id: '1',
...
const mockBrandMetrics: BrandMetrics[] = [
  {
    id: '1',
    period: '2024-01',
    totalReach: 15000,
...
const mockPublicationGoals: PublicationGoal[] = [
  {
    id: '1',
  ...
  persist(
    (set, get) => ({
      ...
      updateContent: (id, updates) => {
        set((state) => ({
          contentQueue: state.contentQueue.map(content =>
            content.id === id 
      ...
      updateVoiceDraft: (id, updates) => {
        set((state) => ({
          voiceDrafts: state.voiceDrafts.map(draft =>
            draft.id === id 
      ...
      updatePublicationGoal: (id, updates) => {
        set((state) => ({
          publicationGoals: state.publicationGoals.map(goal =>
