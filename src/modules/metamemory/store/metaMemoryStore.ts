...
import { MetaMemoryState, ActionLog, MemorySnapshot, Insight } from '../types'
...
const mockSnapshots: MemorySnapshot[] = [
  {
    id: '1',
    date: '2024-02-09',
    type: 'daily',
...
export const useMetaMemoryStore = create<MetaMemoryState>()(
  persist(
    (set, get) => ({
      ...
      toggleStarInsight: (id) => {
        set((state) => ({
          insights: state.insights.map(insight =>
            insight.id === id ? { ...insight, isStarred: !insight.isStarred } : insight
          )
        }))
      },
      ...
      getInsightsByTags: (tags) => {
        return get().insights.filter(insight => 
