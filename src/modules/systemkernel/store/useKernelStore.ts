...
interface KernelState {
  ...
  addOperatingMode: (mode: Omit<OperatingMode, 'id'>) => void
  updateOperatingMode: (id: string, updates: Partial<OperatingMode>) => void
  deleteOperatingMode: (id: string) => void
}

const defaultOperatingModes: OperatingMode[] = [
  {
    id: 'balanced',
    name: 'Balanced Mode',
...
const defaultSystemBehaviors: SystemBehavior[] = [
  {
    id: 'ai-insights',
  ...
  persist(
    (set, get) => ({
      ...
      toggleModule: (moduleId) => {
        set((state) => ({
          moduleStates: state.moduleStates.map(module =>
      ...
      toggleBehavior: (behaviorId) => {
        set((state) => ({
          systemBehaviors: state.systemBehaviors.map(behavior =>
            behavior.id === behaviorId ? { ...behavior, enabled: !behavior.enabled } : behavior
          )
        }))
      },
      ...
      updateOperatingMode: (id, updates) => {
        set((state) => ({
          operatingModes: state.operatingModes.map(mode =>
