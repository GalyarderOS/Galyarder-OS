...
export function usePrivacyAI() {
    ...
    const generateSecurityInsights = () => {
        ...
        const oldItems = vaultItems.filter(item => {
            const daysSinceUpdate = Math.floor((new Date().getTime() - new Date(item.updatedAt).getTime()) / (1000 * 60 * 60 * 24));
        ...
        const expiringSoon = securityKeys.filter(key => {
            if (!key.expiryDate)
                return false;
            const daysUntilExpiry = Math.floor((new Date(key.expiryDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
        ...
        const recentFailures = accessLogs.filter(log => {
            const daysSinceLog = Math.floor((new Date().getTime() - new Date(log.timestamp).getTime()) / (1000 * 60 * 60 * 24));
    ...
    const suggestSecurityImprovements = () => {
        ...
        const accessFrequency = vaultItems.map(item => {
            const daysSinceAccess = Math.floor((new Date().getTime() - new Date(item.lastAccessed).getTime()) / (1000 * 60 * 60 * 24));
