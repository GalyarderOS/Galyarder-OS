...
export const usePrivacyStore = create()(persist((set, get) => ({
    ...
    updateVaultItem: (id, updates) => {
        set((state) => ({
            vaultItems: state.vaultItems.map(item => item.id === id
    ...
    updateSecurityKey: (id, updates) => {
        set((state) => ({
            securityKeys: state.securityKeys.map(key => key.id === id ? { ...key, ...updates } : key)
        }));
    },
    revokeSecurityKey: (id) => {
        set((state) => ({
            securityKeys: state.securityKeys.map(key => key.id === id ? { ...key, status: 'revoked' } : key)
        }));
    },
    ...
    logAccess: (action, itemId, details) => {
        const newLog = {
            id: Math.random().toString(36).substr(2, 9),
            itemId,
            action,
