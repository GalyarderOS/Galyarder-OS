...
export function useKnowledgeAI() {
    ...
    const generateLearningInsights = () => {
        ...
        const stagnantGoals = learningGoals.filter(goal => goal.status === 'active' && goal.progress < 20);
        ...
        const skillsNeedingAttention = skills.filter(skill => {
            const daysSinceLastPractice = Math.floor((new Date().getTime() - new Date(skill.lastPracticed).getTime()) / (1000 * 60 * 60 * 24));
