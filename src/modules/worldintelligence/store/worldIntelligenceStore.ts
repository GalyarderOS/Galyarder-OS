...
import { WorldIntelligenceState, WorldSignal, MacroTrend, StrategicImplication } from '../types'

const mockSignals: WorldSignal[] = [
  {
    id: '1',
...
const mockImplications: StrategicImplication[] = [
  {
    id: '1',
...
export const useWorldIntelligenceStore = create<WorldIntelligenceState>()(
  persist(
    (set, get) => ({
      signals: mockSignals,
      trends: mockTrends,
      ...
      verifySignal: (id) => {
        set((state) => ({
          signals: state.signals.map(signal =>
            signal.id === id ? { ...signal, isVerified: true } : signal
          )
        }))
      },

      addTrend: (trend) => {
        const newTrend: MacroTrend = {
          ...trend,
          id: Math.random().toString(36).substr(2, 9),
          createdAt: new Date().toISOString(),
      ...
      addImplication: (implication) => {
        const newImplication: StrategicImplication = {
          ...implication,
          id: Math.random().toString(36).substr(2, 9),
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString()
      ...
      updateImplication: (id, updates) => {
        set((state) => ({
          implications: state.implications.map(implication =>
            implication.id === id 
