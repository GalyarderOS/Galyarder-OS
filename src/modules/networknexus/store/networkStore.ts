...
import { NetworkState, Contact, Interaction, FollowUp, NetworkGoal } from '../types'
...
const mockNetworkGoals: NetworkGoal[] = [
  {
    id: '1',
    ...
    (set, get) => ({
      ...
      completeFollowUp: (id) => {
        set((state) => ({
          followUps: state.followUps.map(followUp =>
            followUp.id === id 
              ? { ...followUp, status: 'completed' as const, completedAt: new Date().toISOString() }
              : followUp
          )
        }))
      },
      ...
      updateNetworkGoal: (id, updates) => {
        set((state) => ({
          networkGoals: state.networkGoals.map(goal =>
