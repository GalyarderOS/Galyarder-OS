...
const mockInheritanceProtocols = [
    {
        id: '1',
...
export const useDigitalSovereigntyStore = create()(persist((set, get) => ({
    ...
    updateDigitalAsset: (id, updates) => {
        set((state) => ({
            digitalAssets: state.digitalAssets.map(asset => asset.id === id
    ...
    updateOwnershipRecord: (id, updates) => {
        set((state) => ({
            ownershipRecords: state.ownershipRecords.map(record => record.id === id ? { ...record, ...updates } : record)
        }));
    },
    ...
    updateInheritanceProtocol: (id, updates) => {
        set((state) => ({
            inheritanceProtocols: state.inheritanceProtocols.map(protocol => protocol.id === id
    ...
    updateSecurityKey: (id, updates) => {
        set((state) => ({
            securityKeys: state.securityKeys.map(key => key.id === id
