...
import { FamilyMatrixState, FamilyMember, Responsibility, GratitudeEntry, FamilyEvent } from '../types'
...
export const useFamilyMatrixStore = create<FamilyMatrixState>()(
  persist(
    (set, get) => ({
      ...
      updateGratitudeEntry: (id, updates) => {
        set((state) => ({
          gratitudeEntries: state.gratitudeEntries.map(entry =>
      ...
      updateFamilyEvent: (id, updates) => {
        set((state) => ({
          familyEvents: state.familyEvents.map(event =>
            event.id === id 
      ...
      getPendingResponsibilities: () => {
        return get().responsibilities.filter(r => 
