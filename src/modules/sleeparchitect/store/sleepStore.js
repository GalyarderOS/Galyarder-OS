...
export const useSleepStore = create()(persist((set, get) => ({
    ...
    updateCycle: (id, updates) => {
        set((state) => ({
            cycles: state.cycles.map(cycle => cycle.id === id ? { ...cycle, ...updates } : cycle)
        }));
    },
    deleteCycle: (id) => {
        set((state) => ({
    ...
    updateWakeProtocol: (id, updates) => {
        set((state) => ({
            wakeProtocols: state.wakeProtocols.map(protocol => protocol.id === id ? { ...protocol, ...updates } : protocol)
        }));
    },
